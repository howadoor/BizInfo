    ...
    public partial class InfoView : System.Web.UI.UserControl
    {
        ...
        private int GetTotalRowsCount()
        {
            using (SqlConnection connection = new SqlConnection(SqlDataSource.ConnectionString))
            {
                ...
                var whereCommand = InfoViewHelpers.CreateWhereCommand(SearchPhrase);
        ...
        private void UpdateListViewContent(int startRow, int endRow)
        {
            ...
            var pageCommand = string.Format("WITH OrderedOrders AS (SELECT *, ROW_NUMBER() OVER ({1}) AS 'RowNumber' FROM [BizInfo].[dbo].[InfoSetView] {0}) SELECT * FROM OrderedOrders WHERE RowNumber BETWEEN {2} AND {3}", whereCommand, orderCommand, startRow, endRow);
        ...
        protected string GetWebSourceNavigation(Int64? webSourceId)
        {
            ...
            return string.Format("<a class=\"web-source-link\" href=\"{0}\">{1}</a>", webSource.Url, UrlTools.GetServerOnly(webSource.Url));
        }
